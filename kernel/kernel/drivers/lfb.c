...
{
    lfb_vid_memory = (uint8_t*)(uintptr_t)mbi->framebuffer_addr;
    lfb_resolution_x = (uint16_t)mbi->framebuffer_width;
    lfb_resolution_y = (uint16_t)mbi->framebuffer_height;
    lfb_resolution_s = mbi->framebuffer_pitch;
    lfb_resolution_b = mbi->framebuffer_bpp;
    terminal_y = CHAR_HEIGHT * line;

    for( uintptr_t i = (uintptr_t)lfb_vid_memory; i <= (uintptr_t)lfb_vid_memory + lfb_resolution_x * lfb_resolution_y * 4; i += 0x1000 )
    {
